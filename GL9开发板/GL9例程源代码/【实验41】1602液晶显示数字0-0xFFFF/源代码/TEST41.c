...
void main(void)            //主函数,单片机开机后就是从这个函数开始运行
{
    ...
    //**给1602显示寄存器赋值(0x000-0xFFFF)**
        DISbuf=0xA2DE;              //比如说，想显示0xA2DE，直接赋值给它就可以了
    //**************************************

    //***1602液晶显示0x00-0xFFF***
        LCD1602_Disp(0,0,'0');      //在第1行的第1列显示0
        LCD1602_Disp(1,0,'x');      //在第1行的第2列显示x
        j=DISbuf>>12;               //把要显示内容的最高4位右移到最低4位上
        ...
        LCD1602_Disp(2,0,j);        //在第1行的第3列显示最高4位的十六进制数字
        j=(DISbuf>>8)&0x0F;         //把要显示内容的次高4位右移到低4位上，屏蔽其他位
        if(j<10)                    //0-F在ASCII码中并不连续，所以要分开处理
        {
        j+='0';                     //0-9的数据以0为基点进行偏移即可
        }
        ...
        LCD1602_Disp(3,0,j);        //在第1行的第4列显示次高4位的十六进制数字
        j=(DISbuf>>4)&0x0F;         //把要显示内容的次低4位右移到低4位上，屏蔽其他位
        if(j<10)                    //0-F在ASCII码中并不连续，所以要分开处理
        {
        j+='0';                     //0-9的数据以0为基点进行偏移即可
        }
        ...
        LCD1602_Disp(4,0,j);        //在第1行的第5列显示次低4位的十六进制数字
        j=DISbuf&0x0F;              //把要显示内容的高12位屏蔽掉，保留低4位
        if(j<10)                    //0-F在ASCII码中并不连续，所以要分开处理
        {
            j+='0';                 //0-9的数据以0为基点进行偏移即可
        }
        ...
        LCD1602_Disp(5,0,j);        //在第1行的第6列显示低4位的十六进制数字
    //****************************
