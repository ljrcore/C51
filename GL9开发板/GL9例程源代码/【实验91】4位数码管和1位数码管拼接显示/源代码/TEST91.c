...
void main(void)            //主函数,单片机开机后就是从这个函数开始运行
{
    ...
    while(1)               //死循环,单片机初始化后,将一直运行这个死循环
    {
        ...
        if(++d>3) d=0;     //先将d加1，然后判断是否大于3，大于3归零
	 	if(d==0)           //如果d=0,显示万位
        {
            P1=0x03;       //U16A0=1,U16A1=1,U16A2=0选通数码管的万位进行显示
            P2=LED[f/10000];          //将要显示的f的万位提取出来查表后送显示 
        }
        else if(d==1)      //如果d=1,显示千位
        {
            P1=0x02;       //U16A0=0,U16A1=1,U16A2=0选通数码管的千位进行显示
            P2=LED[(f%10000)/1000];    //将要显示的f的千位提取出来查表后送显示
        }
        else if(d==2)      //如果d=2,显示百位
        {
            P1=0x01;       //U16A0=1,U16A1=0,U16A2=0选通数码管的百位进行显示
            P2=LED[(f%1000)/100];      //将要显示的f的百位提取出来查表后送显示
        }
        else             //如果d=3,显示十位
        {
            P1=0x00;       //U16A0=0,U16A1=0,U16A2=0选通数码管的十位进行显示
