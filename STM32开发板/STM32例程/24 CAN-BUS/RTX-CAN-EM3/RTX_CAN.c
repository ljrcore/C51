...
CAN_msgpool_declare(CAN_mpool,
   CAN_CTRL_MAX_NUM*(CAN_No_SendObjects + CAN_No_ReceiveObjects));

//Ô­ÐÍ: #define mbx_arr_declare(name,arr_num,cnt) U32 name[arr_num][4 + cnt]
...
static CAN_ERROR CAN_pull(U32 ctrl, CAN_msg* msg, U16 timeout)
{
   ...
   if (os_mbx_wait(MBX_rx_ctrl[ctrl0], (void * *) &ptrmsg, timeout) ==
      OS_R_TMO)
      return CAN_TIMEOUT_ERROR;
